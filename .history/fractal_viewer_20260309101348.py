...
class FractalApp:
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # Salva i args per uso futuro nello switch 2D/3D
        self.args = args
...
"""
    ...
    def _on_key(self, event) -> None:
        if event.key == "m":
            ...
            # Rimuovi la figura vecchia
            plt.close(self.fig)
            # Ricarica i parametri args con la nuova modalità
            args = argparse.Namespace(
                ...
                y_center=self.y_center,
                zoom=self.zoom,
                julia_cre=self.julia_c.real,
                julia_cim=self.julia_c.imag,
                cmap=_COLORMAPS[self.cmap_index] if self.cmap_index < len(_COLORMAPS) else "inferno",
                fullscreen=self.fullscreen,
                smooth=self.smooth_coloring,
                mode=self.view_mode
            )
            # Crea una nuova app (senza plt.show() che aprirebbe una nuova finestra)
            FractalApp(args)
            return
        elif self.fractal_type == "julia":
            delta = 0.05
